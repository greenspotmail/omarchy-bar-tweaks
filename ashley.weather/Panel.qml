...
Panel {
  ...
  KeyboardPanel {
    ...
    bar: root.bar
    open: root.opened
    // Under the weather icon (right side of the bar), not dead-center on
    // the screen — the panel's own edge clamping pins it to the right
    // margin since the icon already sits there.
    centerOnBar: false
